 ...
 * {@code PageLinkResolver} returned nothing for, and tree macros that fell back to their placeholder
 * token. Fresh per render and driven by a single-threaded traversal, so no synchronization.
 */
final class UnresolvedTracker {
  ...
  UnresolvedReferences build() {
    if (pageIds.isEmpty() && pageTreeRefs.isEmpty()) {
      return UnresolvedReferences.empty();
    }
    return new UnresolvedReferences(pageIds, pageTreeRefs);
  }
}
