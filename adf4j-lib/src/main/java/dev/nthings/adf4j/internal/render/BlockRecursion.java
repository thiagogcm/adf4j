...
/// The recursion entry points a delegate renderer needs to render its children, so it depends on
/// these four callbacks rather than the whole {@link AdfRenderer}. `AdfRenderer` implements it
/// and hands `this` to its delegates, breaking the renderer ⇄ delegate cycle down to one
/// contract.
