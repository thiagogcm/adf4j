...
/// {@link #convert(String, MarkdownOptions)} overload instead of building a new converter each
/// time.
///
/// To render the same document repeatedly (e.g. under different resolvers), parse once and reuse
/// the immutable result instead of paying the JSON parse per render:
/// {@snippet :
/// var parsed = converter.parse(adfJson);
/// var was = converter.convert(parsed, optionsAtT1);
/// var now = converter.convert(parsed, optionsAtT2);
/// }
///
/// The target is GitHub-Flavored Markdown. Some ADF constructs are lossy or by-design (dropped
/// visual marks, the table HTML fallback, synthetic `media:` placeholders).
public final class AdfToMarkdown {
  ...
  /// {@link ContentMetadata#referencedFileIds()} before producing Markdown). Attachment-macro
  /// references resolve against `options.confluenceContext()`, so supply the attachment context
  /// here too. Blank or invalid input yields {@link ContentMetadata#empty()}.
  ...
  /// Extracts {@link ContentMetadata} from an already-parsed {@link AdfDocument} using bound
  /// options.
  /// A `null` document yields {@link ContentMetadata#empty()}.
  public ContentMetadata analyze(@Nullable AdfDocument document) {
    return pipeline.analyze(document, options);
  }

  /// Extracts {@link ContentMetadata} from an {@link AdfDocument} with options supplied for this
  /// call.
  ...
  /// Converts a previously {@link #parse(String) parsed} document to Markdown using the bound
  /// options, carrying the parse issues into the result's diagnostics. Render one
  /// {@link ParseResult} any number of times without re-parsing. A `null`/invalid result yields an
  /// empty body (plus the configured `documentTitle`, if any) with the parse issues preserved.
  ...
  /// Converts an already-parsed {@link AdfDocument} to Markdown using the bound options. A `null`
  /// document yields an empty-body {@link MarkdownResult}. Prefer {@link #convert(ParseResult)}
  /// when the document came from {@link #parse(String)}, so its parse issues reach the result.
  ...
  /// Converts an already-parsed {@link AdfDocument} to Markdown with options supplied for this
  /// call.
  ...
  /// Convenience for `convert(adfJson).body()`.
  public String toMarkdown(@Nullable String adfJson) {
    return convert(adfJson).body();
  }

  /// Convenience for `convert(adfJson, perCallOptions).body()`.
